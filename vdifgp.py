...
from pylab import *
import os, sys, shutil, time, glob, itertools
...
class AROPulsarAnalysis():
    ...
    def __init__(self,):
        ...
        self.psr_name = 'R3'
        self.DM = 348.82 * u.pc / u.cm**3
        self.folder = ('/scratch/p/pen/fleaf5/ARO/2012/Ndisk/20201210T044000Z_aro_vdif/0[0-4]*/*/')
...
#        print('dt', self.dt)
        ...
        self.start_time = fh.start_time + ti * u.min

        if tf == -1: # set the stop time to be fh.stop_time
            self.stop_time = fh.stop_time
        else:
            self.stop_time = fh.start_time + tf *u.min #fh.stop_time #+ 5 * u.min
...
def gp_finder(pa, t, num_samples, gp_thres=5, gp_size=128):
    ...
    print('str(t)',str(t))
    z_raw = pa.process_file(t, num_samples) # the shape should be (time,2pol, 1024freq)
    z = abs(z_raw)**2
...
print('Done the whole search! of '+str(ti)+' to '+str(tf)+' mins')
